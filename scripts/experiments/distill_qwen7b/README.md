# DeepSeek-R1-Distill-Qwen-7B TriAttention Automation
...
1. **Model sync** – `bash scripts/distill_qwen7b/download_models.sh`  
   Downloads only `DeepSeek-R1-Distill-Qwen-7B` into `experiments/models/`.
2. **FullKV traces** – `JOB_PARALLEL=2 bash scripts/distill_qwen7b/run_fullkv.sh`  
   Generates baseline traces for `{aime24,aime25,math500}`; required for stats building.
3. **TriAttention stats** – `bash scripts/distill_qwen7b/build_all_stats.sh`  
   Invokes `scripts/cli.py build-stats` with `--model DeepSeek-R1-Distill-Qwen-7B` and all datasets. The CLI now surfaces batches in dry runs and only launches calibrations when stats files are missing.
4. **Compression sweeps** – run whichever method scripts you need:
   ...
   - `run_one.sh` as a convenience wrapper when testing a single dataset/method/budget (defaults: `dataset=aime24`, `model=DeepSeek-R1-Distill-Qwen-7B`, `method=rkv`).
